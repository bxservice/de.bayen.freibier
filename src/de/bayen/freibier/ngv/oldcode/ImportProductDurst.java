...
//import de.bayen.freibier.model.X_I_ProductDurst;
//
///**
// *	Import Products from I_ProductDurst
// *
// * 	@author 	Jorg Janke
...
// */
//public class ImportProductDurst extends SvrProcess implements ImportProcess
//{
...
//		{
//			sql = new StringBuffer ("DELETE "+getImportTableName()+" "
...
//		sql = new StringBuffer ("UPDATE "+getImportTableName()+" "
...
//		sql = new StringBuffer ("UPDATE "+getImportTableName()+" i "
...
//			+ " AND Value IN (SELECT Value FROM "+getImportTableName()+" ii WHERE i.AD_Client_ID=ii.AD_Client_ID GROUP BY Value HAVING COUNT(*) > 1)").append(clientCheck);
//		no = DB.executeUpdate(sql.toString(), get_TrxName());
//		if (no != 0)
...
//			+ "WHERE I_IsImported<>'Y'"
//			+ " AND UPC IN (SELECT UPC FROM "+getImportTableName()+" ii WHERE i.AD_Client_ID=ii.AD_Client_ID GROUP BY UPC HAVING COUNT(*) > 1)").append(clientCheck);
//		no = DB.executeUpdate(sql.toString(), get_TrxName());
//		if (no != 0)
...
//			+ " (SELECT C_BPartner_ID, VendorProductNo FROM "+getImportTableName()+" ii WHERE i.AD_Client_ID=ii.AD_Client_ID GROUP BY C_BPartner_ID, VendorProductNo HAVING COUNT(*) > 1)")
//			.append(clientCheck);
//		no = DB.executeUpdate(sql.toString(), get_TrxName());
//		if (no != 0)
...
//		// trading unit
//		sql=new StringBuffer("UPDATE "+getImportTableName()+" i "
//				+ "SET BAY_TradingUnit_ID=BAY_TradingUnit.BAY_TradingUnit_ID "
//				+ "FROM BAY_TradingUnit "
//				+ "WHERE BAY_TradingUnit.Value=BAY_TradingUnit_Value " 
//				+ " AND "+getImportTableName()+".AD_Client_ID=" + m_AD_Client_ID
//				);
//		no=DB.executeUpdate(sql.toString(),get_TrxName());
//		if (no != 0)
//			log.info("Number of TradingUnits set =" + no);
//		// prüfen
//		sql=new StringBuffer("UPDATE "+getImportTableName()+" i "
//				+"SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=TradingUnit not found,' "
//				+"WHERE ( "
//				+"SELECT count(*) FROM BAY_TradingUnit "
//				+"WHERE BAY_TradingUnit.Value = I_ProductDurst.BAY_TradingUnit_Value "
//				+") = 0 "
//				+"AND I_ProductDurst.BAY_TradingUnit_Value != '' "
//				+ " AND "+getImportTableName()+".AD_Client_ID=" + m_AD_Client_ID
//				);
//		no=DB.executeUpdate(sql.toString(),get_TrxName());
//		if (no != 0)
//			log.info("Number of wrong TradingUnits =" + no);
//
//		
...
//		sql = new StringBuffer ("SELECT * FROM "+getImportTableName()+" WHERE I_IsImported='N'")
//			.append(clientCheck);
//		try
//		{
...
//				+ " FROM "+getImportTableName()+" WHERE "+getImportTableName()+"_ID=?) "
//				+ "WHERE M_Product_ID=?";
...
//				+ "CostPerOrder,DeliveryTime_Promised "
//				+ "FROM "+getImportTableName()+" "
//				+ "WHERE "+getImportTableName()+"_ID=?", get_TrxName());
//
//			//	Set Imported = Y
//			PreparedStatement pstmt_setImported = DB.prepareStatement
//				("UPDATE "+getImportTableName()+" SET I_IsImported='Y', M_Product_ID=?, "
//				+ "Updated=SysDate, Processed='Y' WHERE "+getImportTableName()+"_ID=?", get_TrxName());
//
//			//
...
//			while (rs.next())
//			{
//				X_I_ProductDurst imp = new X_I_ProductDurst(getCtx(), rs, get_TrxName());
//				int I_ProductDurst_ID = imp.getI_ProductDurst_ID();
//				int M_Product_ID = imp.getM_Product_ID();
//				int C_BPartner_ID = imp.getC_BPartner_ID();
//				boolean newProduct = M_Product_ID == 0;
//				log.fine("I_ProductDurst_ID=" + I_ProductDurst_ID + ", M_Product_ID=" + M_Product_ID 
//					+ ", C_BPartner_ID=" + C_BPartner_ID);
//
//				//	Product
//				if (newProduct)			//	Insert new Product
//				{
//					MProduct product = createNewProduct(imp);
...
//					else
//					{
//						StringBuffer sql0 = new StringBuffer ("UPDATE "+getImportTableName()+" i "
//							+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||").append(DB.TO_STRING("Insert Product failed"))
//							.append("WHERE "+getImportTableName()+"_ID=").append(I_ProductDurst_ID);
...
//						+ " FROM "+getImportTableName()+" WHERE "+getImportTableName()+"_ID="+I_ProductDurst_ID+") "
//						+ "WHERE M_Product_ID="+M_Product_ID;
//					PreparedStatement pstmt_updateProduct = DB.prepareStatement
//						(sqlt, get_TrxName());
//
//					//jz pstmt_updateProduct.setInt(1, I_ProductDurst_ID);
//					//   pstmt_updateProduct.setInt(2, M_ProductDurst_ID);
//					try
//					{
...
//				product.setIsActive(!imp.isDiscontinued());
//				if(imp.getBAY_TradingUnit_ID()>0){
//					MBAYTradingUnit tradingunit = (MBAYTradingUnit) imp.getBAY_TradingUnit();
//					tradingunit.initProduct(product);
//				}
//				product.save();
//
//				//	Do we have PO Info
...
//							+ "CostPerOrder,DeliveryTime_Promised,SysDate,UpdatedBy"
//							+ " FROM "+getImportTableName()+""
//							+ " WHERE "+getImportTableName()+"_ID="+I_ProductDurst_ID+") "
//							+ "WHERE M_Product_ID="+M_Product_ID+" AND C_BPartner_ID="+C_BPartner_ID;
//						PreparedStatement pstmt_updateProductPO = DB.prepareStatement
//							(sqlt, get_TrxName());
//						//jz pstmt_updateProductPO.setInt(1, I_ProductDurst_ID);
...
//						pstmt_insertProductPO.setInt(3, I_ProductDurst_ID);
//						try
//						{
...
//				pstmt_setImported.setInt(2, I_ProductDurst_ID);
...
//
//
//	private MProduct createNewProduct(X_I_ProductDurst imp) {
//		// -TB- angepasste Version des Kommandos:
//		// return new MProduct(imp);
//		//
//		MProduct pr = new MProduct(imp.getCtx(), 0, imp.get_TrxName());
//		pr.set_ValueOfColumn("AD_Client_ID", imp.getAD_Client_ID());
//		pr.setAD_Org_ID(imp.getAD_Org_ID());
//		pr.set_ValueOfColumn("UpdatedBy", imp.getUpdatedBy());
//		//
//		pr.setValue(imp.getValue());
//		pr.setName(imp.getName());
//		pr.setDescription(imp.getDescription());
//		pr.setDocumentNote(imp.getDocumentNote());
//		pr.setHelp(imp.getHelp());
//		pr.setUPC(imp.getUPC());
//		pr.setSKU(imp.getSKU());
//		pr.setC_UOM_ID(imp.getC_UOM_ID());
//		pr.setM_Product_Category_ID(imp.getM_Product_Category_ID());
//		pr.setProductType(imp.getProductType());
//		pr.setImageURL(imp.getImageURL());
//		pr.setDescriptionURL(imp.getDescriptionURL());
//		pr.setVolume(imp.getVolume());
//		pr.setWeight(imp.getWeight());
//		return pr;
//	}
//
//
...
//		return X_I_ProductDurst.Table_Name;
//	}
//
//
//	@Override
